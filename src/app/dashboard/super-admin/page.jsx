...
const OverviewPage = () => (
  <div className="space-y-6 animate-fade-in">
    <h2 className="text-2xl font-bold text-clothcare-navy">Account & System Overview</h2>
    ...
    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
      {[
        ...
        <div key={i} className={`${item.color} p-6 rounded-2xl border border-border-light shadow-clothcareSoft`}>
          <p className="text-text-muted text-sm uppercase font-semibold">{item.label}</p>
          <h3 className="text-3xl font-bold text-clothcare-navy mt-2">{item.val}</h3>
        </div>
      ))}
    </div>

    {/* Big Business Summary */}
    <div className="bg-gradient-to-r from-clothcare-navy to-clothcare-navySoft p-8 rounded-2xl text-white shadow-clothcare">
      <div className="flex justify-between items-center mb-6">
        <h3 className="text-xl font-bold">Today's Business Summary</h3>
        <span className="bg-clothcare-teal/20 text-clothcare-teal px-3 py-1 rounded-full text-sm border border-clothcare-teal">Live Data</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
        ...
        <div className="border-l border-white/10">
          ...
          <p className="text-4xl font-bold text-clothcare-teal">$4,250</p>
        </div>
        <div className="border-l border-white/10">
...
const RequestsPage = () => (
  <div className="space-y-6 animate-fade-in">
    <div className="flex justify-between items-center">
      <h2 className="text-2xl font-bold text-clothcare-navy">Pending Approvals</h2>
    ...
    <div className="bg-bg-white rounded-2xl border border-border-light shadow-sm overflow-hidden">
      {[
        ...
        <div key={i} className="flex items-center justify-between p-6 border-b border-border-light last:border-0 hover:bg-bg-light transition">
          <div>
            <h4 className="font-bold text-clothcare-navy text-lg">{req.name}</h4>
            <p className="text-sm text-text-muted mt-1">{req.type} • <span className="text-clothcare-teal">{req.date}</span></p>
          </div>
          <div className="flex gap-3">
             ...
             <button className="flex items-center gap-2 px-4 py-2 bg-clothcare-teal text-white rounded-xl shadow-clothcareSoft hover:bg-clothcare-tealDark transition">
...
const RevenuePage = () => (
  <div className="space-y-6 animate-fade-in">
    <h2 className="text-2xl font-bold text-clothcare-navy">Financial Overview</h2>
    <div className="p-12 border-2 border-dashed border-border-soft rounded-2xl flex flex-col items-center justify-center text-center bg-bg-soft">
      <Wallet className="w-16 h-16 text-clothcare-teal mb-4" />
      <h3 className="text-xl font-bold text-clothcare-navy">Revenue Charts Component</h3>
...
const FranchisePage = () => (
  <div className="space-y-6 animate-fade-in">
    <div className="flex justify-between items-center">
      <h2 className="text-2xl font-bold text-clothcare-navy">Franchise Management</h2>
      <button className="bg-clothcare-navy text-white px-4 py-2 rounded-xl text-sm font-semibold hover:bg-clothcare-navySoft">+ Add New</button>
    </div>
...
const SuperAdminLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-bg-light font-sans overflow-hidden">
      ...
      {/* 🔹 SIDEBAR (Fixed Left) */}
      <aside className="w-64 bg-clothcare-navy flex flex-col shadow-2xl z-20">
        
        {/* Brand Logo Area */}
        <div className="h-20 flex items-center px-8 border-b border-white/10">
          <div className="w-8 h-8 bg-clothcare-teal rounded-lg mr-3"></div>
          ...
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl transition-all duration-200 group ${
                activeTab === item.id 
                  ? 'bg-clothcare-teal text-white shadow-clothcareSoft' 
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        {/* Top Navbar */}
        <header className="h-20 bg-bg-white border-b border-border-light flex items-center justify-between px-8 shadow-sm">
          <div className="flex items-center gap-4 text-text-primary">
            <Menu className="lg:hidden text-text-muted" /> {/* Mobile trigger placeholder */}
            <h1 className="text-xl font-bold capitalize text-clothcare-navy">
          ...
          <div className="flex items-center gap-6">
             <div className="relative cursor-pointer">
               <Bell className="text-text-muted hover:text-clothcare-teal transition" />
               <span className="absolute -top-1 -right-1 w-2 h-2 bg-status-danger rounded-full"></span>
             </div>
             <div className="flex items-center gap-3">
               <div className="text-right hidden md:block">
                 <p className="text-sm font-bold text-clothcare-navy">Super Admin</p>
                 <p className="text-xs text-text-muted">Master Control</p>
               </div>
               <div className="w-10 h-10 bg-clothcare-tealSoft rounded-full border-2 border-white shadow-sm"></div>
             </div>
          </div>
        </header>
