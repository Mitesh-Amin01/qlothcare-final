...
const RecentHistoryList = () => {
    ...
    return (
        ...
        >
            <h2 className="text-xl font-bold text-text-dark mb-8">Recent Archives</h2>
                ...
                {recentServices.map((service, idx) => (
                    <div key={idx} className="flex items-start gap-4 group cursor-pointer">
                        ...
                        <div className="flex-1 border-b border-gray-100 pb-6 group-hover:border-gray-200 transition-colors">
                            <h4 className="text-text-dark font-bold text-sm leading-tight mb-1 group-hover:text-text-accent transition-colors">{service.name}</h4>
                            <div className="flex items-center justify-between text-xs text-text-muted">
                                <span>{service.date}</span>
                                <span className="font-bold text-text-dark">{service.price}</span>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            <button className="w-full py-4 mt-6 bg-gray-50 hover:bg-gray-100 text-text-dark font-bold rounded-2xl transition-colors text-sm">
                Download Receipts
            </button>
        </motion.div>
    );
